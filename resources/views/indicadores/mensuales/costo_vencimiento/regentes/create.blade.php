...
<x-app-layout>
    ...
    <form action="{{ route('Costo-Vencimientos.store') }}" method="POST" id="myForm" class="bg-white rounded-lg p-6"
        style="box-shadow: 0px 4px 4px rgba(0, 0, 0, 1); "> {{-- /* Sombra del formulario */ --}}
        @csrf
        <div class="text-center">
            <h1><b>REGISTRO MENSUAL DE VENCIMIENTO DEL INVENTARIO HUV</b></h1>
            <p>Recuerda, tienes plazo de responder los 5 primeros dias de cada mes</p> <br><br>
        </div>
        <div class="mb-4">
            <x-label>
                Costo de medicamentos perdidos por vencimiento en el servicio farmaceutico y carros de paro
            </x-label>
        ...
        <div class="mb-4">
            <x-label>
                Costo del inventario del servicio farmaceutico y carros de paro
            </x-label>
    ...
    <div class="mt-4" style="float: right">
        ...
        <x-button class="mt-4 ml-4" style="background-color: rgb(1, 36, 78)">
            <a href="{{ route('Costo-Vencimientos.index') }}">Vista datos mes</a>
        </x-button>
    </div>
