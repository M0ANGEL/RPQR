<x-app-layout>
    ...
    <div class="flex justify-end mb-4">
        <a style="background-color: rgb(23, 16, 165)"
            ...
            href="{{ route('TikectCalificadosDesarrollo.index') }}">CALIFICADOS || FINALIZADOS</a>
        <a style="background-color: rgb(10, 94, 37)"
            ...
            href="{{ route('TikectPendientesCalificadosDesarrollo.index') }}">PENDIENTES POR CALIFICAR</a>
    </div>

    <h1 class="text-white mb-4">Modulo Adminstracion De Ticket Desarrollo</h1>

    {{-- barra buscar --}}
    <form action="{{ route('TicketsAdminDesarrollos.index') }}" method="get">
        <div class="buscar">
    ...
    <div id="marco" class="relative overflow-x-auto" style="border-radius: 6px">
        <table class="w-full text-sm text-center rtl:text-right text-gray-800 dark:text-gray-800"
            ...
            <tbody>
                @foreach ($TikesAdmi as $TicketsAdminDesarrollo)
                    <tr id="filas" class=" border-b  dark:border-gray-700" style="color: rgb(44, 62, 80)">
                        ...
                        <th scope="row" class="px-6 py-4 font-medium text-gray-50 whitespace-nowrap dark:text-white">
                            <b style="color: rgb(44, 62, 80)">{{ $TicketsAdminDesarrollo->id }}</b>
                        </th>
                        {{-- fecha de creacion --}}
                        <td class="px-6 py-4 ">
                            <b style="color: rgb(44, 62, 80)">{{ $TicketsAdminDesarrollo->created_at }}</b>
                        </td>
                        {{-- categoria --}}
                        <td class="px-6 py-4 ">
                            <b style="color: rgb(44, 62, 80)">{{ $TicketsAdminDesarrollo->categoria }}</b>
                        </td>
                        {{-- sun categoria --}}
                        <td class="px-6 py-4">
                            <b style="color: rgb(44, 62, 80)">{{ $TicketsAdminDesarrollo->subcategoria }}</b>
                        </td>
                        {{-- prioridad --}}
                        @switch($TicketsAdminDesarrollo->prioridad)
                            @case(1)
                        ...
                        {{-- numero tike --}}
                        <td class="px-6 py-4">
                            <b style="color: rgb(44, 62, 80)">{{ $TicketsAdminDesarrollo->numero_tike }}</b>
                        </td>
                        {{-- sede --}}
                        <td class="px-6 py-4">
                            <b style="color: rgb(44, 62, 80)">{{ $TicketsAdminDesarrollo->sede }}</b>
                        </td>
                        {{-- estado del tike --}}
                        @switch($TicketsAdminDesarrollo->estado)
                            @case(0)
                                <td class="px-6 py-4">
                        ...
                        @endswitch
                        {{-- cambio de area --}}
                        @if ($TicketsAdminDesarrollo->user_id === null)
                            <td>
                                <button>
                                    <a href="{{ route('CambiarAreaDesarrollo.edit', $TicketsAdminDesarrollo) }}">
                                        <p
                                    ...
                                    @case(0)
                                        <td class="px-6 py-4">
                                            <b style="color: rgb(44, 62, 80)">Ticket abierto por
                                                {{ $TicketsAdminDesarrollo->user->name }}
                                            </b>
                                        </td>
                                    @break
                                    ...
                                    @case(1)
                                        <td class="px-6 py-4">
                                            <b>Ticket abierto por
                                                {{ $TicketsAdminDesarrollo->user->name }} <span
