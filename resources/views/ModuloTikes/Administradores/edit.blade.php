<x-app-layout>
    ...
    <form action="{{ route('AdminTikes.update', $TikectActivo) }}" method="POST"
        class="bg-white rounded-lg p-6 shadow-lg">
        @csrf
        @method('PUT')
        <h1 class="text-center mb-4" style="color: blue">
            <b>Informacion detallada del<br> Ticket {{ $TikectActivo->numero_tike }} </b>
        </h1>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div>
                <x-label>Categoria</x-label>
                <x-input class="w-full" value="{{ $TikectActivo->categoria }}" readonly />
            </div>
            <div>
                <x-label>Sub categoria</x-label>
                <x-input class="w-full" value="{{ $TikectActivo->subcategoria }}" readonly />
            </div>
            <div>
                <x-label>Sedes</x-label>
                <x-input class="w-full" value="{{ $TikectActivo->sede }}" readonly />
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div>
                ...
                <x-input class="w-full" value="{{ $TikectActivo->created_at }}" readonly />
            </div>
            <div>
                <x-label>Usuario que crea el ticket</x-label>
                <x-input class="w-full" value="{{ $TikectActivo->usuario_reporta }}" readonly />
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="mb-4">
                ...
                <textarea id="descripcion" cols="" rows="4"
                    class="border w-full focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" readonly>{{ $TikectActivo->detalle }}</textarea>
            </div>
            {{-- si el ticket ya esta autorizado --}}
            @if ($TikectActivo->autorizacion == 2)
                <div class="mb-4">
                    <x-label><b>Observacion del area encargada de autorizar</b></x-label>
                    <textarea name="respuesta" cols="" rows="4" readonly
                        class="border w-full focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ $TikectActivo->respuesta_autorizacion ? $TikectActivo->respuesta_autorizacion : 'El ticket fue autorizado por el area encargada' }} </textarea>
                </div>
            @endif
    ...
    <form action="{{ route('disponible.edit', $TikectActivo) }}" method="POST" id="FormToken">
        @csrf
        @method('PUT')
