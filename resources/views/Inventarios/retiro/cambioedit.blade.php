...
<x-app-layout>
    <form action="{{ route('cambiarUpdate', $Inventarios_Retiro) }}" method="POST" class="bg-white rounded-lg p-6" style="box-shadow: 0 4px 6px rgba(0, 0, 0, 1);">
        @csrf
        @method('PUT')
        ...
        <div id="datoasubicacion" class="submenu">
            ...
            <div class="equipo_info">
                <x-label style="color: white; text-align: center;">Pareja</x-label>
                <x-input class=" mb-4" readonly value="{{ $Inventarios_Retiro->pareja }}" />
            </div>
        </div>

        <!-- Botón y contenido para Retiro del Equipo -->
        <button type="button" id="botonretiro"><b>CAMBIAR AREA DEL EQUIPO</b></button>
        <div id="datosretiro" class="submenu">
            <div class="equipo_info">
                <x-label style="color: white; text-align: center;">Area nueva del equipo</x-label>
        ...
        <div class="flex justify-end mt-4">
            <x-button style="background: red;">CAMBIAR EL AREA</x-button>
        </div>
    </form>
