...
class HuvSolicitudController extends Controller
{
    ...
    public function reporte(Request $request){
        ...
        $usuarios = DB::table('huvs')->select('id','name','cedula','area','telefono','correo_redv','cargo','bodega','usuario_clonar_huv','cedula_clonar'
        ,'login_servinte','password_servinte','name_referencia')
        ->where('name','like','%'.$busqueda.'%')
        ->where('confirmado','=',true)->where('activo_servinte','=',true)->paginate();
    ...
    /* CODIGO EN CASO DE EL SERVIDOR TENGA OFICCES--
    public function reportePdf(Request $request, huv $huv) {
        // Cargar la plantilla de Word
        $templateProcessor = new TemplateProcessor(storage_path('templates/base_pdf2.docx'));
        ...
        // Guardar el documento rellenado
        $docxFileName = storage_path('app/public/document_' . $data->id . '.docx');
        $templateProcessor->saveAs($docxFileName);
    
        // Convertir el archivo .docx a PDF usando LibreOffice
        $pdfFileName = storage_path('app/public/document_' . $data->id . '.pdf');
        exec("libreoffice --headless --convert-to pdf $docxFileName --outdir " . dirname($pdfFileName));
    
        // Descargar el PDF y eliminar el archivo después de la descarga
        return response()->download($pdfFileName)->deleteFileAfterSend(true);
    }*/
